...
class CosmosDBMigrator:
    ...
    def document_exists(self, document):
        """Vérifie si un document existe déjà dans Cosmos DB"""
        try:
            # Vérifier par application_id (ID unique)
            if "application_id" in document:
                exists = self.collection.find_one({"application_id": document["application_id"]})
                return exists is not None
            # Sinon vérifier par _id
            elif "_id" in document:
                exists = self.collection.find_one({"_id": document["_id"]})
                return exists is not None
            return False
        except Exception:
            return False
    ...
    def insert_document(self, document, index):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Duplicata créé entre-temps - normal, on ignore
                self.stats["duplicates"] += 1
                return True
